    ...
    <div class=" bg-white d-flex justify-center">
        <v-form :class="$vuetify.display.smAndUp ? 'signupFormLg' : 'signupFormSm'" v-model="valid">
            <div class="mt-5 text-right align-center">
                ...
                <h4 class="text-center text-grey mb-4">طور تجارتك بشكل أسرع عبر البيع عن طريق منصة متجري</h4>
                <v-row class="d-flex flex-row-reverse mt-5">
                    <v-col cols="12">
                        <h3 class="mb-3">إسم المتجر</h3>
                    ...
                    <v-col cols="12">
                        <h3 class="mb-3">مجال التجارة</h3>
                        <v-select label="إختر مجال تجارتك" v-model="categorie" :items="store_categrories" :rules="Required" multiple></v-select>
                    </v-col>
                    <v-col cols="12">
...
export default {
    ...
    methods: {
        ...
        register() {
            ...
            const categoryList = this.categorie.map((category) => category.trim());
            const fd = {
                username: this.username,
                phone: this.validatetel,
                categorie:categoryList,
